...
// Add a WooCommerce settings tab for filter selection
function custom_register_filter_settings_tab( $settings_tabs ) {
    $settings_tabs['custom_filters'] = __( 'Custom Filters', 'textdomain' );
    return $settings_tabs;
}
add_filter( 'woocommerce_settings_tabs_array', 'custom_register_filter_settings_tab', 50 );

// Register fields for selecting which filters to display
function custom_register_filter_settings_fields() {
    woocommerce_admin_fields( custom_get_filter_settings_fields() );
}
add_action( 'woocommerce_settings_tabs_custom_filters', 'custom_register_filter_settings_fields' );

// Save the selected filters when settings are updated
function custom_save_filter_settings_fields() {
    woocommerce_update_options( custom_get_filter_settings_fields() );
}
add_action( 'woocommerce_update_options_custom_filters', 'custom_save_filter_settings_fields' );

// Define the settings fields for categories & attributes
function custom_get_filter_settings_fields() {
    ...
    $settings[] = array(
        'name' => __( 'Select Categories for Filters', 'textdomain' ),
        'type' => 'title',
        'desc' => 'Choose which categories should be available as filters.',
        'id'   => 'custom_filter_category_section',
    );
    ...
    foreach ( $categories as $category ) {
        $settings[] = array(
            'name' => $category->name,
            'type' => 'checkbox',
            'id'   => 'custom_filter_category_' . $category->term_id,
            'desc' => __( 'Enable this category filter', 'textdomain' ),
            'default' => 'no',
        );
    }

    $settings[] = array( 'type' => 'sectionend', 'id' => 'custom_filter_category_section' );

    // Attribute Selection Section
    $settings[] = array(
        'name' => __( 'Select Attributes for Filters', 'textdomain' ),
        'type' => 'title',
        'desc' => 'Choose which attributes should be available as filters.',
        'id'   => 'custom_filter_attribute_section',
    );
    ...
    foreach ( $product_attributes as $attribute ) {
        $settings[] = array(
            ...
            'type' => 'checkbox',
            'id'   => 'custom_filter_attribute_' . $attribute->attribute_name,
            'desc' => __( 'Enable this attribute filter', 'textdomain' ),
            'default' => 'no',
        );
    }

    $settings[] = array( 'type' => 'sectionend', 'id' => 'custom_filter_attribute_section' );
